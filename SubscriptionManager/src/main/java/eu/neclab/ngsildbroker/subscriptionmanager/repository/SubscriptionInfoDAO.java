...
public class SubscriptionInfoDAO {
	...
	public Uni<Tuple3<Map<String, Object>, String, Map<String, Object>>> loadSubscription(String tenant, String id) {
		return connectionManager.executeQuery(tenant,
				"SELECT subscription, context, contexts.body as contextBody FROM subscriptions LEFT JOIN contexts ON subscriptions.context = contexts.id WHERE subscription_id=$1",
				Tuple.of(id), false).onItem().transform(rows -> {
					if (rows.size() == 0) {
